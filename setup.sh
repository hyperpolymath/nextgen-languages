...
install_just() {
    ...
    case "$PKG_MGR" in
        ...
        *)
            fail "No supported package manager; install a verified just package before continuing."
            return 1
            ;;
    esac
...
# ── Main ──
main() {
    printf "%s=== nextgen-languages Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/nextgen-languages.git"
        info "Then: cd nextgen-languages && ./setup.sh"
        exit 1
    fi
